...
export const useChessStore = create<ChessStore>((set, get) => ({
  ...
  makeMove: (source, target, promotion = "q") => {
    const { isGameOver, playerColor, socket, history } = get();
    if (isGameOver || !playerColor || game.turn() !== playerColor) return false;
    ...
    try {
      ...
      if (move) {
        ...
        set({
          fen: newFen,
          history: [...history, move.lan],
          moveFrom: "",
          optionSquares: {},
        });

        // Send move to FastAPI so Cerberus or other player can sync
